...
make_basefs() {
    mkarchiso ${verbose} -C /proc/$$/fd/3 -D "${install_dir}" -p "base" create "${work_dir}"
    mkarchiso ${verbose} -C /proc/$$/fd/3 -D "${install_dir}" -p "memtest86+ syslinux" create "${work_dir}"
}

# Additional packages (root-image)
make_packages() {
    ok "Update local repository"
    for i in custompkgs/*xz; do repo-add custompkgs/custompkgs.db.tar.gz $i; done
    mkarchiso ${verbose} -C /proc/$$/fd/3 -D "${install_dir}" -p "$(grep -v ^# packages.i686 | xargs) $(find custompkgs/ -name '*.pkg.tar.xz' | sed -n 's/.*\/\([0-z]\+\)-.*/\1 /p' | xargs)" create "${work_dir}"
}
...
ok make_customize_root_image && make_customize_root_image
