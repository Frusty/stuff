...
Example: $0 jare.es -c "show version" -c "show interface status" -c "show interface trunk"
EOF
exit 1;
}

my %chuis =();
$chuis{'.1.3.6.1.4.1.2467.4.7'}        = {name => 'CSS11501',         user => 'XXXX', pass => 'XXXX' , runcmd => 'terminal length 65535', defcmd => 'show running-config'};
...
$chuis{'.1.3.6.1.4.1.5624.2.2.286'}    = {name => 'C2H124-48P',       user => 'XXXX', pass => 'XXXX'};
...
$chuis{'.1.3.6.1.4.1.2636.1.1.1.2.31'} = {name => 'ex4200-24p',       user => 'XXXX', pass => 'XXXX',  runcmd => 'set cli screen-length 0'};

my @communities = qw/public/;
