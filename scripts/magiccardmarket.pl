...
sub fill_sellers () {
    ...
    my $count = 0;
    while ($content =~ /idInfoUser=\d+">([^<]+)<\/a> \((\d+)\).+?location: ([^']+)'.+?showLanguageChart[^']+'([^']+)'.+?showCardQualityChart[^']+'([^']+)'.+?>(\d+),(\d+)\s[^>]+<\/td><td class="st_ItemCount[^>]+>(\d+)</sg) {
        $count++;
...
sub sellerlist() {
    ...
    my $cut = ($items/3); # I'll be skipping sellers with only 1/3 hits of our total items
    my $ceiling = int($cut) + ($cut != int($cut)); # rounded up
    foreach my $sname (sort { $sellers{$b}{hits} <=> $sellers{$a}{hits} } keys %sellers) {
        if ($sellers{$sname}{hits} <= $ceiling) {
